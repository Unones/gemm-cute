...
from gemm_v2 import _kernel_host_gemm_v2
...
def benchmark_gemm_v2():
    ...
    results = {"size_M": [], "kernel_v2": [], "pytorch": []}
    ...
    for M in sizes_M:
        ...
        compiled = cute.compile(_kernel_host_gemm_v2, a_, b_, c_, d_)
        ...
        results["kernel_v2"].append((flops / ms_kernel) * 1e-9)
        results["pytorch"].append((flops / ms_torch) * 1e-9)

    PEAK_TFLOPS = 87.9 * (2.30 / 2.452)     # TFLOP/s

    plt.plot(results["size_M"], results["kernel_v2"], marker="o", label="Custom CuTe DSL Kernel (v2)")
